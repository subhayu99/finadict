...
def show_forecast(m, forecast, data, p, df_train):
    ...
    delta = only_forecast['Predicted Price'].iloc[-1] - only_forecast['Actual Price'].iloc[-2]
    st.metric(label=" Tomorrow Stock Price", value=only_forecast['Predicted Price'].iloc[-1], delta=delta)
